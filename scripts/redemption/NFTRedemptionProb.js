...
describe('NFT Redemption contract', function () {
  let NFTRedemptionCFactory,
    cdhNftCFactory,
    towerInventoryCFactory,
    equipmentPoolCFactory,
    heroPoolCFactory,
    spellPoolCFactory,
    towerPoolCFactory,
    blackHoleContract,
  ...
  async function deployContract() {
    ...
    NFTRedemption = await NFTRedemptionCFactory.deploy(
      cdhNft.address,
  ...
  for (let rarityProb = 1; rarityProb <= 100; rarityProb++) {
    it('should return rarity for counter=1 in Gold ticket}', async function () {
      const {NFTRedemption} = await loadFixture(deployContract);
      let counter = 1;
  ...
  for (let rarityProb = 1; rarityProb <= 100; rarityProb++) {
    it('should return rarity for counter=3 in Gold ticket}', async function () {
      const {NFTRedemption} = await loadFixture(deployContract);
      let counter = 3;
      let rarity = await NFTRedemption._getProbabilisticRarityGold(rarityProb, counter);
